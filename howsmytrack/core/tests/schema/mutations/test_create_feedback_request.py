...
from howsmytrack.core.schema.mutations.create_feedback_request import (
...
class CreateFeedbackRequestTest(TestCase):
    ...
    def test_logged_out(self):
        ...
        self.assertEqual(
            FeedbackRequest.objects.filter(user=self.user,).count(), 0,
        )
    ...
    def test_soundcloud(self):
        ...
        self.assertEqual(
            result,
            CreateFeedbackRequest(success=True, error=None, invalid_media_url=False,),
        )
    ...
    def test_existing_unassigned_request(self):
        ...
        self.assertEqual(
            FeedbackRequest.objects.filter(user=self.user,).count(), 1,
        )
    ...
    def test_same_url_different_account(self):
        ...
        self.assertEqual(
            FeedbackRequest.objects.filter(
                media_url="https://soundcloud.com/ruairidx/bruno", user=self.user,
            ).count(),
            0,
        )
