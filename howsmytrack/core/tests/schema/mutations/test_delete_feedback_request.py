...
from howsmytrack.core.schema.mutations.delete_feedback_request import (
...
class DeleteFeedbackRequestTest(TestCase):
    ...
    def test_logged_out(self):
        ...
        result = (
            ...
            .fields["deleteFeedbackRequest"]
            .resolver(
                self=Mock(), info=info, feedback_request_id=self.existing_request.id,
            )
        )

        self.assertEqual(
            result, DeleteFeedbackRequest(success=False, error="Not logged in.",)
        )
    ...
    def test_unauthorised(self):
        ...
        self.assertEqual(
            result,
            DeleteFeedbackRequest(
                success=False, error="You are not the owner of this feedback request.",
            ),
        )
    ...
    def test_ok(self):
        ...
        self.assertEqual(result, DeleteFeedbackRequest(success=True, error=None,))

        self.assertEqual(
            FeedbackRequest.objects.count(), 0,
        )
